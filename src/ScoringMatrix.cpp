...
*/
//constructor
ScoringMatrix::ScoringMatrix(int s1size,int s2size, double pen, double gapExt)
:	iLength(s1size),
	jLength(s2size),	
	gapOpening(double(pen)),
	gapExtension(double(gapExt)),
...
void ScoringMatrix::calculateScores(std::vector<std::string> s2, Profile& prf, FeaturesProfile& featPrf,int debug){
	...
	for (unsigned int i = 1; i < matrixV.size();i++){
		for (unsigned int j = 1; j < matrixV.at(i).size(); j++){
			...
			matrixV[i][j] = findVal::maxValueDoubles(score1,score2,score3);
			///G
			score1 = matrixV.at(i-1).at(j) + gapOpening * featPrf.getGapOpeningPenalty(i-1,s2.at(j));
			score2 = matrixG.at(i-1).at(j) + gapExtension;
//			score2 = matrixG.at(i-1).at(j) + gapExtension * featPrf.getGapExtensionPenalty(i-1,s2.at(j));
//			score1 = matrixV.at(i-1).at(j) + gapOpening;
//			score2 = matrixG.at(i-1).at(j) + gapExtension;
			matrixG[i][j] = (score1 > score2) ? score1 : score2;
			///H
			score1 = matrixV.at(i).at(j-1) + gapOpening * featPrf.getGapOpeningPenalty(i-1,s2.at(j));
			score2 = matrixH.at(i).at(j-1) + gapExtension;
//			score2 = matrixH.at(i).at(j-1) + gapExtension * featPrf.getGapExtensionPenalty(i-1,s2.at(j));
//			score1 = matrixV.at(i).at(j-1) + gapOpening;
//			score2 = matrixH.at(i).at(j-1) + gapExtension;
			matrixH[i][j] = (score1 > score2) ? score1 : score2;
		}
	}
...
void ScoringMatrix::nwAlignment(std::vector<std::vector<std::string> > *result,std::vector<std::string> s1,std::vector<std::string> s2, Profile& prf, FeaturesProfile& featPrf,std::string verbose){
	//std::vector<std::string> s2 = misc::pseudoSequence(prf.getMatrix()[0].size()+1); //creating polyA pseudoSequence representing the profile, to know later where are the gaps in the profile
	s1 = vecUtil::push_front(s1,"-AAA");
	s2 = vecUtil::push_front(s2,"-AAA");
	...
	while (i > 0 || j > 0){
		if (i > 0 && j > 0 && currentMatrix == "V"){	//match/mismatch
			...
			newChar1 = s1.at(i);
			newChar2 = "-AAA";
			if (matrixG.at(i).at(j) == matrixV.at(i-1).at(j) + gapOpening * featPrf.getGapOpeningPenalty(i-1,s2.at(j))){
			//if (matrixG.at(i).at(j) == matrixV.at(i-1).at(j) + gapOpening){
				currentMatrix = "V";
			}
			i--;
			...
			newChar1 = "-AAA";
			newChar2 = s2.at(j);
			if (matrixH.at(i).at(j) == matrixV.at(i).at(j-1) + gapOpening * featPrf.getGapOpeningPenalty(i-1,s2.at(j))){
			//if (matrixH.at(i).at(j) == matrixV.at(i).at(j-1)+gapOpening){
				currentMatrix = "V";
			}
			j--;
		}
